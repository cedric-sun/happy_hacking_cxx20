...
struct Foo {
    ...
    static const int scx1 = 142; // in-class init of const static data member is okay; is both declaration and definition because initializer exists
    static const int scx2; // DECLARES a static const data member of complete type const int. Definition provided outside the class definition in the same way of a non-const int, but must with initializer.
...
const int Foo::scx2 = 0; // definition
